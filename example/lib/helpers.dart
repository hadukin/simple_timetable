...
List<Event<TimeTableEvent>> eventCreator(List<TimeTableEvent> listEvents) {
  List<Event<TimeTableEvent>> _events = listEvents.map((item) {
    int startHour = int.parse(item.payload.eventStart.split(':')[0]);
    int startMinutes = int.parse(item.payload.eventStart.split(':')[1]);
    int endHour = int.parse(item.payload.eventEnd.split(':')[0]);
    int endMinutes = int.parse(item.payload.eventEnd.split(':')[1]);
